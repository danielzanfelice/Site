...
app.get("/amizades/pendentes", (req, res) => {
    ...
    const sql = "SELECT a.id, u.usuario AS remetente FROM amizades a JOIN usuarios u ON a.id_remetente = u.id WHERE a.id_destinatario = (SELECT id FROM usuarios WHERE usuario = ?) AND a.status = 'pendente'";
    conexao.query(sql, [usuario], (erro, resultados) => {
        if (erro) {
            console.error(erro);
...
app.get("/amizades/lista", (req, res) => {
    ...
    const sql = "SELECT u.usuario AS nome_amigo FROM amizades a JOIN usuarios u ON (a.id_remetente = u.id OR a.id_destinatario = u.id) WHERE (a.id_remetente = (SELECT id FROM usuarios WHERE usuario = ?) OR a.id_destinatario = (SELECT id FROM usuarios WHERE usuario = ?)) AND a.status = 'aceito' AND u.usuario != ?";
...
app.get("/amizades/status", (req, res) => {
    ...
    const sql = "SELECT status FROM amizades WHERE (id_remetente = (SELECT id FROM usuarios WHERE usuario = ?) AND id_destinatario = (SELECT id FROM usuarios WHERE usuario = ?)) OR (id_remetente = (SELECT id FROM usuarios WHERE usuario = ?) AND id_destinatario = (SELECT id FROM usuarios WHERE usuario = ?))";
...
app.delete("/amizades/desfazer", (req, res) => {
    ...
    const sql = "DELETE FROM amizades WHERE (id_remetente = (SELECT id FROM usuarios WHERE usuario = ?) AND id_destinatario = (SELECT id FROM usuarios WHERE usuario = ?)) OR (id_remetente = (SELECT id FROM usuarios WHERE usuario = ?) AND id_destinatario = (SELECT id FROM usuarios WHERE usuario = ?))";
...
app.get("/mensagens/historico", (req, res) => {
    ...
    const sql = "SELECT id, remetente, destinatario, conteudo, lida, criado_em FROM mensagens WHERE (remetente = ? AND destinatario = ?) OR (remetente = ? AND destinatario = ?) ORDER BY id ASC";
...
app.get("/mensagens/nao-lidas", (req, res) => {
    ...
    const sql = "SELECT remetente, COUNT(*) AS total FROM mensagens WHERE destinatario = ? AND lida = 0 GROUP BY remetente";
    conexao.query(sql, [usuario], (erro, resultados) => {
